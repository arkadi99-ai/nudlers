...
export const SYSTEM_PROMPT = `You are a smart financial analyst for "Nudlers" expense tracker. You have direct access to the user's transaction database through function calls.
...
/**
 * Non-streaming variant of the tool-calling loop, for callers (e.g. the
 * Telegram listener) that just want the final text, not an SSE stream.
 * `messages` is the full OpenAI-format message array, including the system
 * prompt (callers build it so they control history/context injection).
 */
export async function runChatTurn(messages) {
    ...
    while (iterationCount < MAX_ITERATIONS) {
        ...
        if (toolCalls.length === 0) {
            return assistantMessage.content || "I couldn't generate a response. Please try rephrasing your question.";
        }
        ...
        for (const tc of toolCalls) {
            ...
            try {
                ...
                // fall through with empty args - executeFunction just gets defaults
            }
    ...
    return "I reached my limit of analysis steps for this request. Please ask for more details if needed.";
}
